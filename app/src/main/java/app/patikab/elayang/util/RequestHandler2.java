...
public class RequestHandler2 {
    ...
    IParse mIParse;
    String meto = "";
    public RequestHandler2(Context mcontext, IParse iParse) {
        context = mcontext;
        mIParse = iParse;
    }
                    ...
                    public void onResponse(JSONObject response) {
                        try {
                            ...
                            if (response.getBoolean("success")) {
                                mIParse.parseResponses(true, endpoint, response.toString(),meto);
                            } else {
                                if (response.isNull("message")) {
                                    mIParse.parseResponses(true, endpoint, response.toString(),meto);
                                } else {
                                    Toast.makeText(context, response.getString("message"), Toast.LENGTH_LONG).show();
                                    mIParse.parseResponses(false, endpoint, response.toString(),meto);
                                }
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
...
//                            doLogout();
                        ...
                        mIParse.parseResponses(false,  endpoint, Config.DEFAULT_STRING,meto);
    ...
    public interface IParse {
        void parseResponses(Boolean isSuccess, String method, String data,String meto);
    }
